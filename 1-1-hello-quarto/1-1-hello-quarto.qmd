...
participants <- readr::read_csv(
  "https://raw.githubusercontent.com/quarto-rdmss-26/admin/refs/heads/main/data/public/participants-anonymized.csv",
...
participants <- participants |>
  mutate(
    experience_programming_general = factor(
      recode(experience_programming_general, !!!exp_levels),
      levels = unname(exp_levels)
    ),
    experience_programming_r = factor(
      recode(experience_programming_r, !!!exp_levels),
      levels = unname(exp_levels)
    )
  )
```
...
- do [not]{.highlight-yellow} have Git / GitHub experience
...
#| fig-height: 5.5
#| fig-align: center
#| fig-cap: "Coding experience across the cohort (n = 11)"

experience_long <- participants |>
  select(
    `Programming experience\n(any language)` = experience_programming_general,
    `R-specific experience`                  = experience_programming_r
  ) |>
  tidyr::pivot_longer(
    everything(),
    names_to  = "domain",
  ...
  mutate(
    domain = factor(
      domain,
      levels = c("Programming experience\n(any language)", "R-specific experience")
    ),
    level = factor(level, levels = unname(exp_levels), ordered = TRUE)
  ) |>
  count(domain, level, .drop = FALSE)

ggplot(experience_long,
       aes(x = "", y = n, fill = forcats::fct_rev(level))) +
  geom_col(width = 0.6) +
  geom_text(
  ...
  facet_wrap(~ domain, ncol = 1, strip.position = "left") +
  labs(
    x = NULL,
    y = "Number of participants",
    fill = "Experience level"
  ) +
  scale_fill_colorblind(breaks = unname(exp_levels)) +
  ...
  theme(
    ...
    strip.text.y.left = element_text(angle = 0, face = "bold", hjust = 1, size = 13),
...
-   [**3**]{.highlight-yellow} asked for things this course [covers directly]{.highlight-yellow} (personal websites, blogs, Quarto vs. GitHub Pages).
-   [**2**]{.highlight-yellow} asked for things [partially covered]{.highlight-yellow} (Quarto as the documentation layer for R scripts; Quarto as part of a reproducible pipeline).
-   [**6**]{.highlight-yellow} asked for things [outside the scope]{.highlight-yellow} of this course (Git workflows, AI agents for coding, AI with spreadsheets, long-term data archiving).

The other goals are valuable, but not what this workshop is about. I would be happy to follow up for targeted workshops. 
