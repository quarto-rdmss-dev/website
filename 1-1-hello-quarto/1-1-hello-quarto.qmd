...
- **Find these three components** in the file:
  - the YAML header (between the `---` lines at the top)
  - one code chunk (between ```` ```{r} ```` and ```` ``` ````)
  - one Markdown heading (line starting with `##`)
- **Make one change**, save, re-render. Pick either:
  - (a) change `x = bill_length_mm` to `x = flipper_length_mm`, or
  - (b) change `slice_head(n = 10)` to `slice_head(n = 5)`
:::
